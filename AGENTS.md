...
- Page content renders inside a CRT barrel-filter stack (scanlines + SVG `feDisplacementMap` on `#crt-content`); Lenis smooth-scrolls the inner content.
- The custom cursor overlay and Oneko companion use `data-turbo-permanent` with saved state so they stay seamless across Turbo navigations.
