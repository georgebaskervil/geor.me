...
function tsTypeToJSDocument(checker, type, context) {
  ...
  try {
    if (checker.isArrayLikeType && checker.isArrayLikeType(type)) {
      const elementType =
        (checker.getArrayElementType && checker.getArrayElementType(type)) ||
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (!processEverything) {
        ...
        if (cleanId.includes("/dist/") || /\.min\.js$/i.test(cleanId)) return;
      }
      ...
      try {
        ...
        traverse(babelAst, {
          FunctionDeclaration(path) {
            ...
            const tsNode = findTSNodeAtPosition(sourceFile, path.node.id.start);
            if (!tsNode) return;
            let functionType;
            try {
            ...
            const sig = sigs && sigs.length > 0 ? sigs[0] : undefined;
            if (!sig) return;
            ...
            try {
              parameterTypes = sig.parameters.map((parameter) => {
                ...
                const pType = decl
                  ? checker.getTypeOfSymbolAtLocation(parameter, decl)
                  : undefined;
            ...
            const isMeaningful =
              returnType !== "any" || parameterTypes.some((t_) => t_ !== "any");
            ...
            const alreadyHasJSDocument = existingLead.some(
              (c) =>
                c.type === "CommentBlock" &&
                (c.value.includes("@returns") || c.value.startsWith("!")),
            );
            ...
            if (enableCoercions) {
              path.traverse({
                BinaryExpression(subPath) {
                  ...
                  if (
                    parameterNode &&
                    parameterTypes[path.node.params.indexOf(parameterNode)] ===
                      "number" &&
          ...
          VariableDeclarator(path) {
            ...
            const typeString = tsTypeToJSDocument(checker, variableType);
          ...
          ForOfStatement(path) {
            ...
            if (t.isVariableDeclaration(path.node.left)) {
              ...
              if (first && t.isIdentifier(first.id)) leftId = first.id;
            ...
            try {
              elementType =
                ...
                (checker.getElementTypeOfArrayType &&
                  checker.getElementTypeOfArrayType(arrayType)) ||
                undefined;
            } catch {
              /* ignore */
            }
            const elementString = tsTypeToJSDocument(checker, elementType);
              ...
              path.traverse({
                BinaryExpression(subPath) {
                  if (
                    ...
                    subPath.node.left.name === leftId.name &&
                    addTypeCoercion(subPath, "number")
                  )
                    didChange = true;
                },
